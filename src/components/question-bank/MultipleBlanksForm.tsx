...
import { Blank, NormalizedQuestion, Option } from "@/types/question.types";
import {
  useCreateCbtQuestion,
  useUpdateCbtQuestion,
} from "@/hooks/queryHooks/useQuestionBank";
import { multipleBlanksToPayload } from "@/types/question.mapper";
...
export const MultipleBlanksForm = ({
  classId,
  subjectId,
  topicId,
  editQuestion,
  ...
  const updateBlank = (id: string, key: keyof Blank, val: unknown) =>
    setBlanks((prev) =>
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = multipleBlanksToPayload(
        questionText,
        instruction,
        blanks,
        classId,
      ...
      <div className="flex-1 space-y-5 overflow-y-auto px-6 py-5">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4">
          ...
          <p className="mb-2 text-xs text-gray-400">
            Add a brief instruction if needed.
          </p>
          <input
            type="text"
            value={instruction}
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4">
          ...
          <p className="mb-3 text-xs text-gray-400">
            ...
            Blanks are inserted as{" "}
            <code className="rounded bg-gray-100 px-1 text-xs">[Blank N]</code>
          </p>
          <textarea
            value={questionText}
            ...
            <div className="space-y-3">
              {blanks.map((blank, idx) => (
                <BlankConfigCard
                  ...
                  onToggle={() =>
                    setExpandedBlankId(
                      expandedBlankId === blank.id ? null : blank.id,
                    )
                  }
                  onUpdate={(key, val) => updateBlank(blank.id, key, val)}
                  onRemove={() => {
                    setBlanks((prev) => prev.filter((b) => b.id !== blank.id));
                  }}
                />
              ))}
            </div>
...
const BlankConfigCard = ({
  ...
  <div className="overflow-hidden rounded-xl border border-gray-200">
    <div className="flex w-full items-center gap-3 px-4 py-3 transition-colors hover:bg-gray-50">
      ...
      <button
        onClick={onRemove}
        className="flex h-6 w-6 shrink-0 items-center justify-center text-gray-300 transition-colors hover:text-red-500"
      >
    ...
    {isExpanded && (
      <div className="space-y-4 border-t border-gray-100 px-4 py-4">
        ...
        <div className="flex gap-2">
          {(["short-answer", "multiple-choice"] as const).map((ansType) => (
            <button
              key={ansType}
              type="button"
              onClick={() => onUpdate("answerType", ansType)}
              className={cn(
                "flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-medium transition-all",
                blank.answerType === ansType
            ...
            >
              ...
              {ansType === "short-answer" ? "Short Answer" : "Multiple Choice"}
            </button>
          ))}
        </div>
        ...
        {blank.answerType === "multiple-choice" && (
          <BlankOptionsEditor blank={blank} onUpdate={onUpdate} />
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            value={blank.mark}
            onChange={(e) =>
              onUpdate("mark", Math.max(1, Number(e.target.value)))
            }
...
// ─── Blank options editor (for multiple-choice blanks) ────────────────────────

const BlankOptionsEditor = ({
  blank,
  onUpdate,
}: {
  blank: Blank;
  ...
  const options: Option[] = blank.options ?? [
    ...
    <div className="space-y-1.5">
      {options.map((opt, j) => (
        <div key={opt.id} className="flex items-center gap-2">
          ...
          <span className="w-5 text-xs font-medium text-gray-400 uppercase">
            {opt.id}.
          </span>
          <input
            type="text"
