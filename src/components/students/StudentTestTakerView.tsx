...
export const StudentTestTakerView = ({ assessmentId }: { assessmentId: string }) => {
	...
	const isComprehension = currentSection.isComprehension || currentSection.isImageBased;
		...
		<div className="flex h-screen flex-col overflow-hidden bg-gray-50">
			...
			<header className="flex h-14 shrink-0 items-center justify-between border-b border-gray-200 bg-white px-6">
				...
				<div className="flex items-center gap-3">
					<div className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-semibold ${timeLeft < 300 ? "border-red-200 bg-red-50 text-red-600" : "border-gray-200 text-gray-700"}`}>
						⏱ {formatTime(timeLeft)}
					</div>
					<button
			...
			<div className="flex min-h-0 flex-1">
				...
				<div className="flex min-h-0 flex-1 flex-col">
					...
					<div className="border-b border-gray-100 bg-white px-6 py-4">
						<h2 className="text-base font-bold text-gray-900">{currentSection.title}</h2>
						<p className="mt-0.5 text-xs text-gray-400">Instructions (optional)</p>
					</div>
							...
							<div className="flex w-1/2 shrink-0 flex-col overflow-y-auto border-r border-gray-100 bg-white px-6 py-5">
								{currentSection.isImageBased ? (
									<>
										...
										<p className="text-xs italic text-gray-400">Instructions (optional)</p>
									</>
								) : (
									<>
							...
							<div className="flex-1 overflow-y-auto px-6 py-5">
								...
								<div className="space-y-4">
									{currentSection.questions.map((q, idx) => {
										const globalNum = sections
											.slice(0, sectionIdx)
											.reduce((s, sec) => s + sec.questions.length, 0) + idx + 1;
										return (
											<QuestionRenderer
												key={q.id}
												question={q}
					...
					<div className="flex h-14 shrink-0 items-center justify-between border-t border-gray-100 bg-white px-6">
						...
						<span className="text-xs text-gray-400">
							...
							<span className="font-medium text-gray-600">{answeredCount}/{totalQuestions} answered</span>
						</span>
