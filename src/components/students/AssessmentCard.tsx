...
import { Clock, FileText, Calendar, RotateCcw, Play, CheckCircle2, AlertTriangle, XCircle } from "lucide-react";
...
const GradedBody = ({ score, totalMarks }: { score: number; totalMarks: number }) => (
...
const ActionButton = ({ card }: { card: StudentAssessmentCard }) => {
	const base = "flex w-full items-center justify-center gap-2 rounded-lg py-2 text-sm font-medium transition-colors";

	if (card.status === "in-progress") {
		return (
			<Link href={`/students/${card.id}/take`} className={cn(base, "border border-gray-200 text-gray-700 hover:bg-gray-50")}>
	...
	if (card.status === "not-started") {
		return (
			<Link href={`/students/${card.id}`} className={cn(base, "bg-blue-600 text-white hover:bg-blue-700")}>
	...
	if (card.status === "scheduled") {
		return (
			<button disabled className={cn(base, "border border-gray-100 bg-gray-50 text-gray-300 cursor-not-allowed")}>
				Not Open
			</button>
		);
	}
	if (card.status === "graded") {
		return (
			<Link href={`/students/${card.id}`} className={cn(base, "border border-gray-200 text-gray-700 hover:bg-gray-50")}>
				View Result
			</Link>
		);
	}
...
export const AssessmentCard = ({ card }: { card: StudentAssessmentCard }) => (
	<div className="flex flex-col rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
		...
		<div className="mb-2 flex items-center justify-between">
			<span className={cn("text-xs font-semibold", SUBJECT_COLORS[card.subjectColor] ?? "text-blue-600")}>
				{card.subject}
			</span>
			<span className={cn("rounded-full px-2 py-0.5 text-xs font-medium", STATUS_BADGE[card.status])}>
		...
		{card.status === "graded" && card.score !== undefined && card.totalMarks !== undefined && (
