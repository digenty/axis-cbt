      ...
      <div className="flex flex-col h-screen bg-white overflow-hidden">
                ...
                <div className="flex h-full">
                  ...
                  <div className="w-[42%] border-r border-gray-100 overflow-y-auto px-6 py-5 text-sm text-gray-700 leading-relaxed text-justify">
                    {hasPassage ? (
                      <>
                        ...
                        />
                        {!firstQ.passage && (
                          <p className="whitespace-pre-wrap">{DEMO_PASSAGE}</p>
                        )}
                      </>
                    ) : (
                  ...
                  <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                    <p className="text-xs text-gray-500 mb-2">
                      {firstQ.instruction || "Answer the following questions"}
                    </p>
                    {(firstQ.subQuestions?.length
                      ...
                      <QuestionCard
                        ...
                        onAnswerChange={(patch) => updateAnswer(q.id, patch)}
        ...
        <TotalScoreModal
          ...
          onUpload={async (feedback) => {
            ...
            router.push(`/classes/${classId}/subjects/${subjectId}/results`);
          }}
        />
      </div>
    </Layout>
    ...
    <div className="border border-gray-200 rounded-xl p-4 bg-white">
      ...
      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
        ...
        <input
          ...
          onChange={(e) =>
            onMarkChange(
              Math.min(question.marks, Math.max(0, Number(e.target.value))),
            )
          }
    ...
    <div className="space-y-4">
      ...
      <div className="text-sm leading-loose flex flex-wrap items-center gap-1">
        {blanks.length > 0 ? (
          blanks.map((blank, idx) => {
            ...
            const prevText =
              idx === 0 ? question.text?.split(/Blank \d+/)[0] || "" : "";
            return (
              <span key={blank.id} className="inline-flex items-center gap-1">
                {idx === 0 && <span className="text-gray-700">{prevText}</span>}
      ...
      {blanks.length > 0 && (
        <div className="border border-gray-200 rounded-xl p-4 bg-gray-50/50">
          <h4 className="text-xs font-semibold text-gray-700 mb-3">Blanks</h4>
          <div className="space-y-4">
            {blanks.map((blank, idx) => {
                      ...
                      <div className="space-y-2">
                        {blank.options?.map((opt) => {
                          ...
                          return (
                            <button
                              key={opt.id}
                              onClick={() => updateBlank(blank.id, opt.id)}
                              className={cn(
                                "w-full flex items-center gap-3 px-4 py-2.5 rounded-xl border text-sm text-left transition-all",
