        ...
        <div className="group flex items-center gap-2 px-4 py-3">
          ...
          <div className="min-w-0 flex-1 cursor-pointer" onClick={onToggle}>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              {(qType === "QUESTION_GROUP" || qType === "COMPREHENSION") &&
                (() => {
          ...
          {question?.options?.map((opt) => (
            ...
            >
              ...
              <span className="flex-1">
                {opt?.optionText || <em className="text-gray-300">No text</em>}
              </span>
            </div>
          ))}
        </div>
      ...
      {question?.questionType === "FILL_IN_THE_BLANK" && (
        <div className="space-y-1.5">
          {question?.blanks && question?.blanks.length > 0 ? (
            ...
            <p className="text-xs italic text-gray-400">No blanks defined</p>
          )}
        </div>
      )}
      ...
      {question?.questionType === "MATCH" && (
        <div className="space-y-1.5">
          {question?.pairs && question?.pairs?.length > 0 ? (
            ...
            <p className="text-xs italic text-gray-400">No pairs defined</p>
          )}
        </div>
      )}
          ...
          {(question as any)?.subQuestions?.length > 0 && (
            <div className="space-y-1">
              <p className="text-xs font-medium text-gray-500">
                ...
                {(question as any)?.subQuestions?.length !== 1 ? "s" : ""}
              </p>
              {(question as any)?.subQuestions?.map((sq: any, i: number) => (
                <div
                  key={i}
                ...
                >
                  ...
                  <span className="shrink-0 text-gray-400">{sq?.marks} mk</span>
                </div>
              ))}
            </div>
          )}
