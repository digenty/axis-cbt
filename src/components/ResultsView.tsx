...
import { useGetAssessmentResults, useGetAssessmentStats } from "@/hooks/queryHooks/useAssessment";
import { useGetClassDetails } from "@/hooks/queryHooks/useSubjects";
import type { ApiAssessment, AssessmentStudentResult, StudentResultStatus } from "@/types/question";
...
export const ResultsView = ({
	...
	const effectiveSelectedId = selectedAssessmentId ?? assessments[0]?.id ?? null;
						...
						(results.length > 0
							? Math.round(
									(results
										...
										Math.max(1, results.filter((r) => r.score !== null).length)) *
										10,
								) / 10
							: 0)
					}
